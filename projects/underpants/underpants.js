...
_.filter = function(arr, func){
    var filtered = [];
    _.each(arr, function(element, index, coll){
        if(func(element, index, coll)) filtered.push(element);
    });
    return filtered;
};
...
_.reject = function(arr, func){
    var rejected = [];
    _.each(arr, function(element, index, coll){
        if(!func(element, index, coll)) rejected.push(element);
    });
    return rejected;
};
...
_.partition = function(arr, func){
    var partitioned = [[], []];
    _.filter(arr, function(element, index, coll) {
        if(func(element, index, coll)){
...
_.unique = function(arr){
    ...
    _.each(arr, function(element, index, coll){
        if(_.indexOf(duplicatesRemoved, element) === -1) {
...
_.map = function(coll, func){
    ...
    _.each(coll, function(element, index, coll){
        mapped.push(func(element, index, coll));
    });
    return mapped;
};
...
_.pluck = function(arr, prop){
    var plucked = [];
    _.map(arr, function(element, index, coll){
        plucked.push(element[prop]);
    });
    return plucked;
};
...
_.extend = function(objectA, objectB){
    ...
    for(var i = 1; i < arguments.length; i++){
        _.each(arguments[i], function(val, key, coll){
            result[key] = val;
        });
    }
    return result;
